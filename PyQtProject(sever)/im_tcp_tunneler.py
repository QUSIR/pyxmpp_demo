...
def send_xmpp_cmd(client,from_jid, to_jid, txt):raise NotImplemented
def addsever(self,data,to_jid, from_jid):raise NotImplemented
def closesever(self,to_jid, from_jid,num):raise NotImplemented
def closeseveror(self,to_jid, from_jid,num):raise NotImplemented
...
#创建监听函数  sock服务器端
def s2x_socket_listener(self,(src_addr, src_port), (dst_addr, dst_port), dst_jid):
    sock = socket.socket()
    ...
    while 1:
        ...
        if get_jid_keyid(c.remote_jid):
            ...
            ek = '-'
        #通过XMPP发送数据
        send_xmpp_message(self.client,get_client_jid(self.client), c.remote_jid,
    ...
    try:
        ...
        #关闭链接
         ...
         #数据
            ...
            data.append(sever)
            '''
            #self.client.append(Client(JID('b@localhost'), '123456',None))
           # addsever(self,data,to_jid, from_jid)
            addsever(self,sendata,to_jid, from_jid)
            #data='OK !'+number
            #send_xmpp_cmd(self.client,to_jid, from_jid, data)
        elif body.startswith('CMD CLOSE'):
            print 'delete port'
            num=int(body[body.find('!')+1:])
            #sendata=num.split('!')
            closesever(self,to_jid, from_jid,num)
            #num='DELETE !'+str(num)
            #send_xmpp_cmd(self.client,to_jid, from_jid, num)
        elif body.startswith('CMD OR CLOSE'):
            print 'delete port'
            if body.find('!')!=-1:
                num=(body[body.find('!')+1:])
                numlist=[]
                sendata=num.split('!')
                for i in range(len(sendata)):
                    numlist.append(int(str(sendata[i])))
                #编写同时删除
                closeseveror(self,to_jid, from_jid,numlist)
        elif body.startswith('TESTLINK'):
            print 'testlink'
            send_xmpp_message(self.client,to_jid,from_jid,'TESTLINKOK')
        if resp is not None:
...
"""读取配置文档 """
def setup_tunnels(self,filename):
    ns = {}
