...
def getaddrinfo(host, port, family = None,
    ...
    for rtype, rfamily in rtypes:
        ...
        logging.debug('link xmpp 30 1 ')
        #er=str(r.rrset.items[0].address)
        #er=r.rrset.items[0].address
        #er=r.to_text()
        #logging.debug(er)
        logging.debug('link xmpp 30 2 ')
        #print r.rrset.items[0].address
        if r:
            for rr in r:
                #rr=str(rr)
                #rr=rr.decode(rr)
                ...
                if i!=-1:
                    ...
                    logging.debug(j)
                    de=j[j.find(' ')+1:]
                    logging.debug('i find')
                    logging.debug(de)
                    #ip=str(de)
                    ip=long(de,16)
                    #ip = socket.inet_ntoa(struct.pack('I',socket.htonl(de)))
                    li=ch1(ip)
                    #li=int(de)
                    li=str(li)
                    logging.debug(li)
                ...
                logging.debug(li)
                #cun=str(cun)
                #cun=cun.decode('utf-8')
                #cun=cun.encode('utf-8')
                #cun=cun.decode('utf-8')
                #sdf=('127.0.0.1',5222)
                sd=(li,port)
                #logging.debug(sdf)
